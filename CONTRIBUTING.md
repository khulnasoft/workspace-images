...
You need to have a khulnasoft account in order to seamlessly contribute to this repo.
...
This repo is already configured with a [.khulnasoft.yml](.khulnasoft.yml) which installs and sets up all the required dependency.
...
1. Make sure the user `khulnasoft` can access the installed tool/lang.
1. The last `USER` command of the image should always be `khulnasoft` and **NOT** `root`.
...
e.g. `RUN echo "/etc/mydb/mydb-bashrc-launch.sh" >> /home/khulnasoft/.bashrc.d/100-my-service-launch.sh`
...
1. Open a Khulnasoft workspace with your Pull Request
...
1. Create a `.khulnasoft.yml` and a corresponding `khulnasoft.Dockerfile`:
    ...
    ```Dockerfile
    # test/khulnasoft.Dockerfile
    FROM localhost:5000/dazzle:full # replace "full" with your own
    # ... optionally, more steps here
    ```

1. Now you can test using [`gp validate`](https://www.khulnasoft.com/docs/references/khulnasoft-cli#validate).
